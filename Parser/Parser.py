...
#----------------helpers----------------------
    ...
    def consume(self, token_type):
        if not self.check(token_type):
            raise Errors.LoomParseError(
                f"Expected {token_type}, found {self.peek().value}"
            )
...
#-----------------Parser modules---------------------------------
    ...
    def parse_statement(self):
        ...
        if self.check(Token.TokenType.PRINT):
        ...
        elif self.check(Token.TokenType.TAG):
            return self.parse_tag()

        elif self.check(Token.TokenType.NAME):

            # tag();
            if self.peek().value == "tag":
                return self.parse_tag_statement()
            ...
            raise Errors.LoomParseError(
                f"Expected: print/return/if/while/def/Block/tag/"
                f"variable/function call\n"
                f"Got: {self.peek().value}"
            )

    def parse_tag_statement(self):
        self.consume(Token.TokenType.NAME)
        self.consume(Token.TokenType.LPAREN)
        self.consume(Token.TokenType.RPAREN)
        self.consume(Token.TokenType.SEMICOLON)

        return AST.TagStatement()
...
#-------------------parse_arith()----------------------------------
    ...
    def parse_primary(self):
        ...
        if self.check(Token.TokenType.NUMBER):
            ...
            if token.value == "tag":
                raise Errors.LoomParseError(
                    "tag() cannot be used as an expression"
                )
            ...
            raise Errors.LoomParseError(
                f"Expected Expression found {self.peek().type}"
            )
    ...
    def parse_term(self):
        ...
        while (
            self.check(Token.TokenType.STAR)
...
#----------------------------------------------------------------
    ...
    def parse_func_def(self):
        ...
        if not self.check(Token.TokenType.RPAREN):
            ...
            args.append(
                self.consume(Token.TokenType.NAME).value
            )
    ...
    def parse_block(self):
        ...
        while (
            not self.check(Token.TokenType.RBRACE)
...
#-------------------------Annotations------------------------------
    ...
    def parse_tag(self):
        ...
        tag = self.consume(Token.TokenType.TAG)

        # @tag { ... }
        if self.check(Token.TokenType.LBRACE):

            self.consume(Token.TokenType.LBRACE)

            annotations = []

            while not self.check(Token.TokenType.RBRACE):
        ...
        return AST.AnnotationStatement(
            tag.tag,
            tag_bodystr,
            body
        )